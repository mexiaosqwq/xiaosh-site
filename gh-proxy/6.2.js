...
(() => {
  ...
  // 图片请求的 Accept 头，让代理端能据此判定为图片而非附件下载。
  const IMAGE_ACCEPT =
  ...
  function scheduleRevocation(element, attributeName, values) {
    ...
    // 直接持有 state.objectURLs 的引用，回收后据此判断能否清空状态。
    // 注意：调用方在调度后不得再原地修改该数组。
   ...
   * data 模式的候选挑选。
   *
   * 这是降级路径，不追求与浏览器一致的精确选择：
   * 密度候选取最接近当前 DPR 的，宽度候选取最大的。
   * srcset 规范本身也不允许 w 与 x 混用。
   */
  function pickBestCandidate(candidates) {
    ...
    for (const candidate of candidates) {
      if (!isProxyableValue(candidate.url)) {
        continue;
      }

      const descriptor = candidate.descriptor || '';
      const widthMatch = descriptor.match(/(\d+(?:\.\d+)?)\s*w/i);
      const densityMatch = descriptor.match(/(\d+(?:\.\d+)?)\s*x/i);

      let score;

      if (densityMatch) {
        score = -Math.abs(Number(densityMatch[1]) - dpr);
      } else if (widthMatch) {
        score = Number(widthMatch[1]) / 1e6;
      } else {
        score = -Math.abs(1 - dpr);
      }
  ...
  /**
   * <source> 的 media 是否命中当前环境。
   *
   * type 无法可靠探测格式支持，这里保守放行。
   */
  function sourceMediaMatches(source) {
  ...
  async function proxySrcsetAttribute(element, attributeName = 'srcset') {
    ...
    const promise = (async () => {
      try {
         ...
         *
         * 注意：本函数只在 blob 模式下调用。
         * data URL 自带逗号，无法作为 srcset 候选，见 data 模式分支。
         */
        ...
        for (const item of loaded) {
          if (item.target && item.blob && item.url) {
            ...
            // 理论上此处必为 blob 模式；若意外拿到 data URL 则跳过，
            // 避免把带逗号的值塞进 srcset。
            if (materialized && materialized.value.startsWith('blob:')) {
            ...
            if (materialized && !materialized.revocable) {
              // data URL：无法进 srcset，保留原候选。
              outputCandidates.push({
  ...
  // ============================================================
  // data 模式：srcset 收敛为单个 src
  // ============================================================

  /**
   * 把 srcset 置空。
   *
   * 目的是阻止浏览器绕过已代理的 src 直接去取原始候选。
   * 空 srcset 的 <img> 会回落到 src，空 srcset 的 <source> 会被跳过。
   */
  function neutralizeSrcset(element) {
    syncAttributeState(element, 'srcset');

    const state = getAttributeState(element, 'srcset');
    const logical = getLogicalValue(element, 'srcset', state);
    ...
    // 已经中和过。
    if (state.applied === '' && !state.revoked) {
      return;
    }
    ...
    commitAttribute({
      ...
      domValueAtStart: element.getAttribute('srcset'),
      logical,
      outputValue: '',
      revocables: []
    });
  ...
  /**
   * 把指定 URL 代理后写入 img.src。
   *
   * logical 记录的是被选中的候选地址，而非元素原本的 src，
   * 这样后续重新代理会继续沿用同一候选。
   */
  async function proxySrcFromURL(image, rawURL) {
    ...
    const state = getAttributeState(image, 'src');

    if (
      state.applied !== null &&
      !state.revoked &&
      state.logical === rawURL
    ) {
      return true;
    }
  ...
  /**
   * data 模式的 img/picture 处理。
   *
   * srcset 的候选以逗号分隔，而 data URL 自身必带逗号，
   * 两者语法不兼容，因此只能收敛成单个 src。
   *
   * 代价：<picture> 的深浅色与 DPR 选择由脚本在此刻一次性决定，
   * 之后切换主题不会自动换图。
   */
  async function proxyImageFamilyAsDataURL(image) {
    const sources = getPictureSources(image);

    let chosen = null;

    // 按 <picture> 的原始优先级找第一个命中的来源。
    for (const source of sources) {
      if (!sourceMediaMatches(source)) {
        continue;
      }

      syncAttributeState(source, 'srcset');

      const state = getAttributeState(source, 'srcset', false);
      const logical = getLogicalValue(source, 'srcset', state);

      if (!logical) {
        continue;
      }

      chosen = pickBestCandidate(parseSrcset(logical));

      // 命中的 source 即为浏览器会选用的那个，
      // 无论其中有没有需要代理的候选都不再向后查找。
      break;
    }

    if (!chosen) {
      syncAttributeState(image, 'srcset');

      const state = getAttributeState(image, 'srcset', false);
      const logical = getLogicalValue(image, 'srcset', state);
    ...
    // 无论是否选中候选，都要把所有 srcset 置空，
    // 否则浏览器会绕过 src 直接去取原始候选。
    for (const source of sources) {
      neutralizeSrcset(source);
    }

    neutralizeSrcset(image);

    if (chosen) {
      await proxySrcFromURL(image, chosen.url);
      return;
    }

    // 没有可代理的候选，退回处理原始 src。
    if (attributeNeedsProxy(image, 'src', 'url')) {
      await proxyURLAttribute(image, 'src');
    }
  }
  ...
  async function proxyImageFamily(image) {
    ...
    // data 模式：srcset 与 data URL 语法冲突，收敛为单 src。
    if (mode === 'data') {
      await proxyImageFamilyAsDataURL(image);
      return;
    }

    // blob 模式：src 与各 srcset 分别代理，保留响应式语义。
    const tasks = [];
